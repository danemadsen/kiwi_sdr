...
class ImaAdpcmDecoder {
  static const List<int> _stepSizeTable = [
  ...
  static const List<int> _indexAdjustTable = [-1, -1, -1, -1, 2, 4, 6, 8, -1, -1, -1, -1, 2, 4, 6, 8];

  static int _clamp(int x, int xmin, int xmax) {
  ...
  int _decodeSample(int code) {
    ...
    _index = _clamp(_index + _indexAdjustTable[code], 0, _stepSizeTable.length - 1);
    ...
    int sample = _clamp(_prev + difference, -32768, 32767);
    _prev = sample;
    return sample;
  }
